...
def decide(current, approved):
    ...
    delta = current["cv_mean"] - approved["cv_mean"]

    if delta >= THRESHOLD:
        return {
            "status": "accepted",
            "accepted": True,
            "delta_cv_mean": float(delta),
            "reason": f"Improved CV mean by {delta:.5f}, meeting threshold {THRESHOLD:.5f}."
        }
    ...
    return {
        ...
        "accepted": False,
        "delta_cv_mean": float(delta),
        "reason": f"Improvement {delta:.5f} did not meet threshold {THRESHOLD:.5f}."
    }
...
def append_research_log(current_run):
    ...
    baseline_val = (
        f"{current_run['approved_baseline']['cv_mean']:.5f}"
        if current_run["approved_baseline"] is not None
        else ""
    )
    delta_val = (
        f"{current_run['decision']['delta_cv_mean']:.5f}"
        if current_run["decision"]["delta_cv_mean"] is not None
        else ""
    )
    ...
    line = (
        ...
        f"| {current_run['metrics']['cv_mean']:.5f} "
        f"| {current_run['metrics']['cv_std']:.5f} "
        f"| {baseline_val} "
        f"| {delta_val} "
        f"| {THRESHOLD:.5f} "
...
def main():
    ...
    x_train, y_train, x_test = build_feature_matrices(train_df, test_df, feature_set=feature_set)
